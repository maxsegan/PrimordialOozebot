...
bool ParetoFront::evaluateEncoding(OozebotEncoding encoding) {
    auto encodingFront = this->encodingFront;
    ...
    int fitnessBucket = round(encoding.fitness / fitnessBucketSize);
    ...
    auto iter = encodingFront.begin();
    while (iter != encodingFront.end()) {
        ...
        if (dominates(frontEncoding, encoding)) {
            ...
            iter = encodingFront.erase(iter); // this dominates one - it will certainly be added but also may dominate others
        } else {
            ++iter;
        }
    }
    encodingFront.push_back(encoding);
    return true;
}
...
void ParetoFront::resize() {
    ...
    this->buckets = {};
    this->ageBucketSize = this->maxAge / 100;
    this->fitnessBucketSize = round(this->maxFitness / 100);
